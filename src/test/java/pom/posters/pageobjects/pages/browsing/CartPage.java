...
import pom.posters.pageobjects.pages.checkout.ShippingAddressPage;
import pom.posters.pageobjects.utility.PriceHelper;
...
{
    private final SelenideElement cartTable = $("#cartOverviewTable");

    private final SelenideElement subTotal = $("#orderSubTotalValue");
    ...
    public void validateShippingCosts(final String shippingCosts)
    {
    ...
    public void validate(final String shippingCosts)
    {
        validateStructure();
    ...
    public void validateCartItem(final int position, final Product product)
    {
     ...
     */
    @Step("validate \"{product}\" in on the cart page")
    public void validateCartItem(final int position, final Product product, final int productAmount)
    {
        validateCartItem(position, product.getName(), product.getStyle(), product.getSize(), productAmount,
                product.getUnitPrice());
    }

    private void validateCartItem(final int position, final String productName, final String productStyle,
            final String productSize, final int productAmount, final String productPrice)
    {
        final SelenideElement productContainer = $("#product" + (position - 1));
        // Visibility
    ...
    public void validateSubAndLineItemTotalAfterAdd(final int position, final String oldSubTotal,
            final String oldLineItemTotal)
    {
        final SelenideElement productContainer = $("#product" + (position - 1));
        // Store unit price (without $ sign)
        // Takes the pricer per 1 unit of the specified item
        final String unitPriceShort_varDynamic = productContainer.find(".unitPriceShort").text();
        // Store product count
        // Takes the amount of the specified item
        final String quantity_varDynamic = $("#productCount" + (position - 1)).val();

        final String subOrderPrice = PriceHelper.computeRowPrice(PriceHelper.addCurrency(unitPriceShort_varDynamic),
                quantity_varDynamic);
        ...
        final String newSubTotal = subTotal.text();
        // New Total - Old Total = Price of item you just added
        final String price = PriceHelper.subtractFromPrice(newSubTotal, oldSubTotal);
        final String price2 = PriceHelper.subtractFromPrice(subOrderPrice, oldLineItemTotal);

        Assert.assertEquals(price, price2);
    }

    @Step("validate line item amount on the cart page")
    public void validateProductAmount(final int position, final int amount)
    {
        // Makes sure the amount of the item with index @{index} in the cart equals the
        // parameter
    ...
    public String getProductName(final int position)
    {
    ...
    public String getProductStyle(final int position)
    {
    ...
    public String getProductSize(final int position)
    {
    ...
    public String getProductCount(final int position)
    {
        // Get the size to enable usage outside this module.
    ...
    public String getProductUnitPrice(final int position)
    {
    ...
    public String getProductTotalUnitPrice(final int position)
    {
        // Get the product price to enable usage outside this module.
    ...
    public Product getProduct(final int position)
    {
        return new Product(getProductName(position), getProductUnitPrice(position), getProductTotalUnitPrice(position),
                getProductStyle(position), getProductSize(position), Integer.parseInt(getProductCount(position)));
    };
    ...
    public void updateProductCount(final int position, final int amount)
    {
        final SelenideElement productContainer = $("#product" + (position - 1));
    ...
    public void validateSubAndLineItemTotalAfterRemove(final String oldSubTotal, final String oldLineItemTotal)
    {
        final String newSubTotal = PriceHelper.subtractFromPrice(oldSubTotal, oldLineItemTotal);
    ...
    public ProductdetailPage openProductPage(final int position)
    {
